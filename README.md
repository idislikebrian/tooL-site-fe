...
`NEXT_PUBLIC_APP_URL` is used for Mini App metadata, manifests, and wallet app metadata. It defaults to `https://tool-site-fe.vercel.app` and is normalized without a trailing slash.

### Farcaster Mini App

This app includes Farcaster Mini App support as an adaptation layer around the existing website.

The root page emits an `fc:miniapp` meta tag. The manifest endpoint is available at `/.well-known/farcaster.json`, but it intentionally returns `404` until a real signed account association is configured.

Do not deploy a placeholder or fake `accountAssociation` to production. Generate the signed object in Farcaster Developer Tools for the exact production domain, including subdomain, then set it as JSON:
